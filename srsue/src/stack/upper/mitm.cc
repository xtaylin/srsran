...
#include "srsue/hdr/stack/upper/mitm.h"
...
void mitm::write_pdu(srsran::unique_byte_buffer_t& pdu)
{
  ...
  if (header.lcid < SRSRAN_N_SRB) {
    rrc->parse_ul_dcch(header.lcid, sdu);
  }

  if (sdu.get() && (sdu->N_bytes > 0)) {
    pdcp->write_sdu(header.lcid, std::move(sdu));
  }
...
void mitm::write_sdu(uint32_t lcid, srsran::unique_byte_buffer_t sdu)
{
  mitm_header_t header = {0};
